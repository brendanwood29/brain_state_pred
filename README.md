...
4. Run `uv sync` to install the other dependencies
5. To train a new single subject model, first prepare your data in a folder with the suffix `*timseries.csv` and run `uv run train_single_subject.py configs/single_subject_config.yaml`
6. To train a new pre-trained model, first prepare your data using the `prepare_data.py` and `make_datasplits.py` scripts, then using `uv run train.py configs/config.yaml` train a new model.
